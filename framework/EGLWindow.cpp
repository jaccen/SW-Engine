...
EGLPlatformParameters::EGLPlatformParameters()
	: renderer(EGL_PLATFORM_SW_DEVICE_TYPE_NULL),
...
bool EGLWindow::initializeGL(Window *osWindow)
{
	...
	//displayAttributes.push_back(EGL_PLATFORM_SW_TYPE_SW);
	displayAttributes.push_back(mPlatform.renderer);
	//displayAttributes.push_back(EGL_PLATFORM_SW_MAX_VERSION_MAJOR_SW);
	displayAttributes.push_back(mPlatform.majorVersion);
	//displayAttributes.push_back(EGL_PLATFORM_SW_MAX_VERSION_MINOR_SW);
	displayAttributes.push_back(mPlatform.minorVersion);

	if (mPlatform.deviceType != EGL_DONT_CARE)
	{
		//displayAttributes.push_back(EGL_PLATFORM_SW_DEVICE_TYPE_SW);
	...
	if (mPlatform.presentPath != EGL_DONT_CARE)
	{
		...
		if (strstr(extensionString, "EGL_SW_experimental_present_path") == nullptr)
		{
			destroyGL();
			return false;
		}

		//displayAttributes.push_back(EGL_EXPERIMENTAL_PRESENT_PATH_SW);
		displayAttributes.push_back(mPlatform.presentPath);
	}
	displayAttributes.push_back(EGL_NONE);

	mDisplay = eglGetPlatformDisplayEXT(0,
		reinterpret_cast<void *>(osWindow->get_native_window()),
		&displayAttributes[0]);
	...
	mSurface = eglCreateWindowSurface(mDisplay, mConfig, osWindow->get_native_window(), &surfaceAttributes[0]);
	if (eglGetError() != EGL_SUCCESS)
	{
		destroyGL();
		return false;
